...
bool CSimpleLog::InitLogFile()
{
	...
	if (access("dll_log", 0)) {
		std::string dirName = "dll_log";
		bool flag = CreateDirectory((LPCWSTR)dirName.c_str(), NULL);
	}
	...
	SYSTEMTIME st;
	GetLocalTime(&st);
	swprintf(m_szLog, L"dll_log\\cv_%d%02d%02d-%02d%02d%02d.log", st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);
	m_nWriteSize = 0;
	return true;
}
